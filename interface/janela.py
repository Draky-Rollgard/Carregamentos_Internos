...
def iniciar():
    ...
    topo = tk.Frame(screen, bg="#1c2b39", height=58)
    topo.pack(fill="x")
    ...
    lateral_esquerda = tk.Frame(screen, bg="#f4f4f4", width=175)
    lateral_esquerda.pack(side="left", fill="y")

    lateral_direita = tk.Frame(screen, bg="#f4f4f4", width=205)
    lateral_direita.pack(side="right", fill="y")

    centro = tk.Frame(screen, bg="#b9e3ed")
    ...
    def atualizar_reacoes_desenho(reacoes):
        if hasattr(elementos_viga, "reacoes"):
        ...
        if reacoes["tipo"] == "engaste":
            ...
            dados_reacoes.append((reacoes["apoio_1"].posicao, reacoes["R1"]))
            dados_reacoes.append((reacoes["apoio_2"].posicao, reacoes["R2"]))

        if hasattr(elementos_viga, "definir_reacoes"):
            elementos_viga.definir_reacoes(dados_reacoes)
        elif hasattr(elementos_viga, "adicionar_reacao"):
            for posicao, valor_reacao in dados_reacoes:
                elementos_viga.adicionar_reacao(posicao, valor_reacao)
        else:
            elementos_viga.atualizar()

    criar_card(area_cards, "reacao_1", "Reação 1")
    criar_card(area_cards, "reacao_2", "Reação 2")
    criar_card(area_cards, "cortante", "Força Cortante")
