...
class ElementosViga:
    def __init__(self, ax, canvas, comprimento=10.0, referencial=0.0):
        self.ax = ax
        self.canvas = canvas
    ...
    def para_exibicao(self, posicao_interna):
        return float(posicao_interna) - float(self.referencial)
    ...
    def configurar(self):
        ...
        ref = float(self.referencial)
        x_inicio = -ref
        x_fim = L - ref
        margem = max(0.08 * L, 2.0)

        self.ax.set_xlim(x_inicio - margem, x_fim + margem)
        self.ax.set_ylim(-3.7, 4.6)
        self.ax.set_xlabel("Posição em relação ao referencial (m)")
        self.ax.set_yticks([])
        self.ax.grid(True, axis="x", linestyle="--", alpha=0.25)
        self.ax.axvline(0, linestyle=":", linewidth=0.9, color="#555555", alpha=0.75)
    ...
    def desenhar_viga(self):
        ...
        self.ax.plot([x_inicio, x_fim], [altura / 2, altura / 2], color="black", linewidth=2, solid_capstyle="round", zorder=3)
        self.ax.plot([x_inicio, x_fim], [-altura / 2, -altura / 2], color="black", linewidth=2, solid_capstyle="round", zorder=3)
        self.ax.plot([x_inicio, x_inicio], [-altura / 2, altura / 2], color="black", linewidth=1, zorder=3)
        self.ax.plot([x_fim, x_fim], [-altura / 2, altura / 2], color="black", linewidth=1, zorder=3)

    def desenhar_pino(self, x):
        self.ax.scatter([x], [-0.5], marker="^", s=400, facecolors="#1f77b4", edgecolors="black", linewidths=1.4, zorder=4)
        self.ax.scatter([x], [-0.9], marker="_", s=950, c="black", linewidths=1.1, zorder=4)

    def desenhar_rolete(self, x):
        self.ax.scatter([x], [-0.5], marker="o", s=400, facecolors="lightgray", edgecolors="black", linewidths=1.0, zorder=5)
        self.ax.scatter([x], [-0.9], marker="_", s=850, c="black", linewidths=1.1, zorder=4)

    def desenhar_engaste(self, x):
        self.ax.plot([x, x], [-2.0, 2.0], linewidth=10, color="gray", zorder=4)
        for y in [-1.7, -1.2, -0.7, -0.2, 0.3, 0.8, 1.3, 1.8]:
            self.ax.plot([x, x - 0.9], [y, y - 0.22], color="black", linewidth=0.7, zorder=4)
    ...
    def desenhar_reacao(self, x, valor):
        ...
        if valor >= 0:
            self.ax.annotate(
                ...
                xy=(x, -0.95),
                xytext=(x, -2.65),
                arrowprops=dict(arrowstyle="-|>", color="red", lw=1.6, mutation_scale=16, shrinkA=0, shrinkB=0),
                zorder=6,
            )
            y_texto = -3.00
        else:
            ...
            y_texto = -0.70
        self.ax.text(x, y_texto, f"{valor:.2f} N", ha="center", fontsize=8, fontweight="bold", color="red")
    ...
    def desenhar_carga_concentrada(self, x, intensidade):
        ...
        if intensidade >= 0:
            self.ax.annotate(
                ...
                xy=(x, 0.35),
                xytext=(x, 3.60),
                arrowprops=dict(arrowstyle="-|>", color=cor, lw=1.6, mutation_scale=16, shrinkA=0, shrinkB=0),
                zorder=5,
            )
        else:
            self.ax.annotate(
        ...
        self.ax.text(x, 3.95, f"P={intensidade:g}", ha="center", fontsize=8, color=cor)
    ...
    def desenhar_carga_constante(self, x1, x2, intensidade):
        ...
        for i in range(n + 1):
            ...
            self.ax.annotate(
                ...
                xy=(xi, y_base),
                xytext=(xi, y_topo),
                arrowprops=dict(arrowstyle="-|>", color=cor, lw=1.1, mutation_scale=12, shrinkA=0, shrinkB=0),
                zorder=5,
            )
    ...
    def desenhar_carga_linear(self, x1, x2, q1, q2):
        ...
        self.ax.plot(xs, ys, color=cor, linewidth=2.0, zorder=5)
        self.ax.text((x1 + x2) / 2, max(ys) + 0.35, "q(x)", ha="center", fontsize=8, color=cor)
    ...
    def desenhar_momento(self, x, intensidade):
        ...
        if intensidade >= 0:
            self.ax.annotate(
                ...
                xy=(x + 0.55, 1.15),
                xytext=(x - 0.55, 1.15),
                arrowprops=dict(arrowstyle="-|>", color=cor, lw=1.8, mutation_scale=16, connectionstyle="arc3,rad=1.2"),
                zorder=5,
            )
        else:
            self.ax.annotate(
        ...
        self.ax.text(x, 1.90, f"M={intensidade:g}", ha="center", fontsize=8, color=cor)
    ...
    def atualizar(self):
        ...
        for tipo, posicao in self._iterar_apoios():
            x = self.para_exibicao(posicao)
            tipo_normalizado = str(tipo).lower()
            if "pino" in tipo_normalizado:
                self.desenhar_pino(x)
            elif "rolete" in tipo_normalizado:
                self.desenhar_rolete(x)
            elif "engast" in tipo_normalizado:
                self.desenhar_engaste(x)

        for posicao, valor in self._iterar_reacoes():
            self.desenhar_reacao(self.para_exibicao(posicao), valor)

        for tipo, dados in self._iterar_cargas():
            if tipo == "concentrada":
                self.desenhar_carga_concentrada(self.para_exibicao(dados["x"]), dados["valor"])
            elif tipo == "constante":
                self.desenhar_carga_constante(
                    self.para_exibicao(dados["x1"]),
                    self.para_exibicao(dados["x2"]),
                    dados["valor"],
                )
            elif tipo == "linear":
                ...
                self.desenhar_momento(self.para_exibicao(dados["x"]), dados["valor"])
