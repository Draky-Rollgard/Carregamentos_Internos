...
def _primeira_posicao_do_valor(xs, ys, valor):
...
def _resumo_extremos(xs, vs, ms):
    ...
    return {
        "vmax": (v_max, _primeira_posicao_do_valor(xs, vs, v_max)),
        "vmin": (v_min, _primeira_posicao_do_valor(xs, vs, v_min)),
        "mmax": (m_max, _primeira_posicao_do_valor(xs, ms, m_max)),
        "mmin": (m_min, _primeira_posicao_do_valor(xs, ms, m_min)),
    }


def _formatar_extremo(valor, posicao, unidade):
    return f"{valor:.2f} {unidade} em x={posicao:g} m"
...
def _atualizar_cards(xs, vs, ms):
    resumo = _resumo_extremos(xs, vs, ms)
    cards_vars["vmax"].set(_formatar_extremo(*resumo["vmax"], "N"))
    cards_vars["vmin"].set(_formatar_extremo(*resumo["vmin"], "N"))
    cards_vars["mmax"].set(_formatar_extremo(*resumo["mmax"], "N.m"))
    cards_vars["mmin"].set(_formatar_extremo(*resumo["mmin"], "N.m"))
...
def _plotar_diagrama(ax, xs, ys, titulo, ylabel, cor):
    ...
    ax.axhline(0, color="#333333", linestyle="--", linewidth=0.9, alpha=0.75)
    ax.axvline(0, color="#555555", linestyle=":", linewidth=0.9, alpha=0.75)
    ax.set_title(titulo, fontsize=11, fontweight="bold", pad=12)
    ax.set_xlabel("x em relação ao referencial (m)", fontsize=9)
...
def _converter_xs_para_referencial(viga, xs):
    if hasattr(viga, "posicao_para_referencial"):
        return [viga.posicao_para_referencial(x) for x in xs]
    return xs


def _atualizar_graficos(viga, xs, vs, ms):
    global fig_diagramas, canvas_diagramas

    xs_plot = _converter_xs_para_referencial(viga, xs)
...
def abrir_janela_diagramas(parent, viga, xs=None, vs=None, ms=None):
    ...
    if janela_existe:
        _atualizar_cards(xs_plot, vs, ms)
        _atualizar_graficos(viga, xs, vs, ms)
    ...
    tk.Label(
        topo,
        text="Força Cortante (V) e Momento Fletor (M) — eixo x no referencial escolhido",
        bg="#1c2b39",
        fg="#d9e2ec",
        font=("Arial", 9),
    ).pack()
    ...
    resumo = _resumo_extremos(xs_plot, vs, ms)
    _criar_card(area_cards, "vmax", "Força Cortante Máx (+)", _formatar_extremo(*resumo["vmax"], "N"))
    _criar_card(area_cards, "vmin", "Força Cortante Mín (-)", _formatar_extremo(*resumo["vmin"], "N"))
    _criar_card(area_cards, "mmax", "Momento Fletor Máx (+)", _formatar_extremo(*resumo["mmax"], "N.m"))
    _criar_card(area_cards, "mmin", "Momento Fletor Mín (-)", _formatar_extremo(*resumo["mmin"], "N.m"))
    ...
    canvas_diagramas = FigureCanvasTkAgg(fig_diagramas, master=frame_graficos)
    canvas_diagramas.get_tk_widget().pack(fill="both", expand=True, padx=10, pady=10)
    ...
    ttk.Button(rodape, text="Fechar", command=ao_fechar).pack(side="right")
